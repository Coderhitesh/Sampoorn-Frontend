...
<head>
    ...
    <style>
        ...
        .toast { position: fixed; top: 20px; right: 20px; padding: 15px; border-radius: 5px; color: white; z-index: 1000; }
        .toast-success { background-color: #10b981; }
        .toast-error { background-color: #ef4444; }
        .loading { display: none; }
        .image-preview { max-width: 200px; max-height: 150px; margin: 10px 0; border: 1px solid #ddd; }
        ...
        .remove-preview { position: absolute; top: -5px; right: -5px; background: red; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer; }
        .hidden { display: none; }
    </style>
</head>
  ...
  <script>
                    ...
                    try {
                        ...
                        if (!field.value.trim()) {
                            ...
                            if (fieldName === 'email' && !isValidEmail(field.value)) {
                                ...
                                logWarning('Invalid email format', { email: field.value });
                            }
                            ...
                            if (fieldName === 'Password' && field.value.length < 6) {
                                ...
                                logWarning('Password too short', { length: field.value.length });
                            }
                        }
                    } catch (error) {
            ...
            return safeExecute(() => {
                ...
                if (input.files && input.files.length > 0) {
                    if (input.files.length > 10) {
                        logWarning('Too many files selected', { count: input.files.length });
                    ...
                    Array.from(input.files).slice(0, 10).forEach((file, index) => {
                        if (!file.type.startsWith('image/')) {
                            logWarning('Non-image file selected', { fileName: file.name, type: file.type });
                            showToast(`File "${file.name}" is not an image and will be skipped`, 'warning');
                            return;
                        }

                        if (file.size > 5 * 1024 * 1024) { // 5MB limit
                            logWarning('File too large', { fileName: file.name, size: file.size });
        ...
        function previewSingleImage(input, previewId) {
            return safeExecute(() => {
                ...
                if (input.files && input.files[0]) {
                    ...
                    if (!file.type.startsWith('image/')) {
                        logError('Selected file is not an image', { fileName: file.name, type: file.type });
                    ...
                    if (file.size > 5 * 1024 * 1024) { // 5MB limit
                        logError('File too large', { fileName: file.name, size: file.size });
    ...
    if (field) {
        ...
        console.log(`❌ ${fieldName} not found in DOM`);
                ...
                const arrayFields = [
                    { container: 'ownerNames', name: 'ownerName' },
                    { container: 'coverageAreas', name: 'coverageArea' },
                    { container: 'coverageAreaDescriptions', name: 'coverageAreaDescription' },
                    { container: 'typeOfVehicles', name: 'typeOfVehicles' },
                    { container: 'channelsOfOperation', name: 'channelsOfOperation' },
                    { container: 'typesOfOperation', name: 'typesOfOperation' },
                    { container: 'businessOperations', name: 'businessOperations' }
                ];
                ...
                arrayFields.forEach(({ container, name }) => {
                    ...
                    if (containerEl) {
                        ...
                        if (values.length > 0) {
                            ...
                            //logInfo(`Added ${name} array to form data`, values);
                        }
                    }
                });
